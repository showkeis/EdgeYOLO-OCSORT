...
points = [] # Global List for mouse point
cross_count = {} # cross count obj Dict.
total_count = {'AtoB': 0, 'BtoA': 0}
...
def is_line_crossed(p1, p2, p3, p4):
    """
    Check if line (p1, p2) intersects with line (p3, p4)
    """
    return np.sign(np.cross(p2 - p1, p3 - p1)) != np.sign(np.cross(p2 - p1, p4 - p1))

def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None,classes=None):
    ...
    shift_y = 15  # shift amount in the y-direction
    ...
    if len(points) >= 2:
        line_p1 = np.array(points[0])
        line_p2 = np.array(points[1])
        cv2.line(im, tuple(line_p1), tuple(line_p2), (200,0,0), thickness=2)  # Moved inside the if-statement

    for i, tlwh in enumerate(tlwhs):
        x1, y1, w, h = tlwh
        intbox = tuple(map(int, (x1, y1, x1 + w, y1 + h)))
        intcenterbox = tuple(map(int, (x1 + w // 2, y1 + h // 2)))  # center coordinate
        lowcenterbox = tuple(map(int, (x1 + w // 2, y1 + h )))  # center coordinate
        ...
        if len(vec_new[id_text]) >= 2:
            p1 = np.array(vec_new[id_text][-2])
            p2 = np.array(vec_new[id_text][-1])
            # If line points have been defined

            if len(points) >= 2:
                if is_line_crossed(line_p1, line_p2, p1, p2):
                    if id_text not in cross_count:
                        cross_count[id_text] = {'AtoB': 0, 'BtoA': 0}
                    direction = 'AtoB' if np.cross(p2 - p1, line_p2 - line_p1) > 0 else 'BtoA'
                    cross_count[id_text][direction] += 1
                    total_count[direction] += 1  # トータルカウントを増加
                    if int(id_text) in cross_count:
                        print(f'Object {int(id_text)} ({class_names[int(classes[i].item())]}) crossed from {direction}. Total: {cross_count[int(id_text)]}')
                    else:
                        print(f'Object {int(id_text)} ({class_names[int(classes[i].item())]}) crossed from {direction}. Not found in cross_count')

                    # Add information text
                    info_text = f'ID: {int(id_text)}, Class: {class_names[int(classes[i].item())]}, Direction: {direction}, Count: {cross_count[id_text]}'
                    ...
                    # Draw the text on the image
                    draw = ImageDraw.Draw(im_pil)
                    draw.text((intbox[0]+shift_x, intbox[1]+shift_y), info_text, font=font, fill=tuple([int(c) for c in color]))
    ...
    # Draw the text on the image
    draw = ImageDraw.Draw(im_pil)
    total_info_text = f'⇐Left: {total_count["BtoA"]}, Right⇒: {total_count["AtoB"]}'
    draw.text((0, 20), total_info_text, font=font, fill=(0, 0, 200))
    ...
    while source.isOpened() and success:
        ...
        for _ in range(BATCH):
            ...
            if not success:
                ...
                print("Stream Load OK.")
                frames.append(frame)

        if not len(frames):
            break

        results = detect(frames, LEGACY)[0] # torch [person number, 7] 7 ==> x1y1x2y...cls num
        """
            Bounding boxの左上のx座標
            Bounding boxの左上のy座標
            Bounding boxの右下のx座標
        ...
        # Initialize OCSort tracker
            ...
            if results is not None:
                imgs = plot_tracking(
                    frames, online_tlwhs, online_ids, frame_id=frame_id + 1, fps=1. / timer.average_time, classes=results[:, -1]
                )
            else:
                # resultsがNoneの場合の処理を記述します。以下は一例です。
                imgs = plot_tracking(
